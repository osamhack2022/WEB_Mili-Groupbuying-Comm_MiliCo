...
const Item = (props) =>{
    ...
    const enroll = async () => {
        const result = await axios.post(`/rest/members/${item.id}`, {UserId: props.login});
        if(result){
        ...
        <>  
            <div className="grouping-header-img-wrapper">
                ...
                <div className="grouping-header-title-wrapper">
                    <Typography className="grouping-header-type">{item.field}</Typography>
                ...
                <Row gutter={128}>
                    <Col offset={2} span={13} className="gutter-row">
                        ...
                        <div className="grouping-page-descr-wrapper">
                            ...
                            <p className="grouping-page-descr-td"> {item.due_money} </p>
                    ...
                    <Col span={9}>
                        ...
                        <Row>
                            <Col span={12}>
                                <Statistic.Countdown valueStyle={{fontSize:"1.5rem"}} title="모집 종료까지" value={item.due_money} format="D일 H시 m분 s초" />
                            </Col>
                            <Col span={12}>
                            ...
                            <Col>
                                {
                                    ...
                                    ? '' 
                                    : 
                                    <Statistic valueStyle={{fontSize:"1.5rem"}} title="모집책임자" value={`${item.Payer.User.unit.split(' ').reverse()[0]} ${item.Payer.User.name} | ${item.Payer.User.trust_index}`} suffix={<LikeOutlined />} />  
                                }
                            </Col>               
                        </Row>
                        <Divider></Divider>
                            ...
                            <Col span={24}>
                                {
                                    ...
                                    auth==2 ?
                                    <Button onClick={()=>{deleteItem}} style={{width:"100%", height: "70px"}}>어그러뜨리기</Button> :
